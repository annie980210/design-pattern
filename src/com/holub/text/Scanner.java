 ...
 * with a Scanner. Here's a stripped-down version:
 * <p>
 ...
 * 
 * private static final Token COMMA = tokens.create("',"), EQUAL = tokens.create("'="), LP = tokens.create("'("),
 * 		RP = tokens.create("')"), DOT = tokens.create("'."), STAR = tokens.create("'*"), SLASH = tokens.create("'/"),
 * 		AND = tokens.create("'AND"), BEGIN = tokens.create("'BEGIN"), CREATE = tokens.create("'CREATE"),
 * 		// ...
 * 		INTEGER = tokens.create("(small|tiny|big)?int(eger)?"), IDENTIFIER = tokens.create("[a-zA-Z_0-9/\\\\:~]+");
 * </PRE>
 * 
...
public class Scanner {
	...
	/**
	 * Create a Scanner for the indicated token set, which will get input from the
	 * indicated string.
	 */
	...
	public Scanner(TokenSet tokens, Reader inputReader) {
		this.tokens = tokens;
		this.inputReader = (inputReader instanceof BufferedReader) ? (BufferedReader) inputReader
	...
	/**
	 * Advance the input to the next token and return the current token (the one in
	 * the input before the advance). This returned token is valid only until the
	 * next <code>advance()</code> call (at which time the lexeme may change, for
	 * example).
	 */
	 ...
	 * position. This method lets a parser that's using the current scanner report
	 * an error in a way that identifies where in the input the error occurred.
	 * 
	...
	/**
	 * Combines the match and advance operations. Advance automatically if the match
	 * occurs.
	 * 
	 * @return the lexeme if there was a match and the input was advanced, null if
	 *         there was no match (the input is not advanced).
	 */
	...
	/**
	 * If the specified candidate is the current token, advance past it and return
	 * the lexeme; otherwise, throw an exception with the error message "XX
	 * Expected".
	 * 
	...
	public static class Test {
		...
		private static final Token COMMA = tokens.create("',"), IN = tokens.create("'IN'"),
				INPUT = tokens.create("INPUT"), IDENTIFIER = tokens.create("[a-z_][a-z_0-9]*");
