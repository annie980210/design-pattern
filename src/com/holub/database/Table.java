...
public interface Table extends Serializable, Cloneable {
	...
	/**
	 * Return the table name that was passed to the constructor (or read from the
	 * disk in the case of a table that was loaded from the disk.) THis is a
	 ...
	 * naming the anonymous table that's returned from {@link #select select(...)}
	 * or one of its variants.
	 */
	void rename(String newName);

	/**
	 * Return true if this table has changed since it was created. This status isn't
	 * entirely accurate since it's possible for a user to change some object that's
	 * in the table without telling the table about the change, so a certain amount
	 * of user discipline is required. Returns true if you modify the table using a
	 * Table method (like update, insert, etc.). The dirty bit is cleared when you
	 * export the table.
	 */
	boolean isDirty();
	 ...
	 * For example, the value at <code>values[i]</code> is put into the column
	 * specified in <code>columnNames[i]</code>. Columns that are not specified are
	 * initialized to <code>null</code>.
	 *
	 * @return the number of rows affected by the operation.
	 * @throws IndexOutOfBoundsException One of the requested columns doesn't exist
	 *                                   in either table.
	 */
	...
	/**
	 * Update cells in the table. The {@link Selector} object serves as a visitor
	 * whose <code>includeInSelect(...)</code> method is called for each row in the
	 * table. The return value is ignored, but the Selector can modify cells as it
	 * examines them. Its your responsibility not to modify primary-key and other
	 * constant fields.
	 * 
	 * @return the number of rows affected by the operation.
	 */
	 ...
	 *
	 * @param all if false, commit only the innermost transaction, otherwise commit
	 *            all transactions at all levels.
	 * @see #THIS_LEVEL
	 * @see #ALL
	 */
	 ...
	 * {@link #rollback} more readable when used as an argument to those methods.
	 ...
	 * A convenience constant that makes calls to {@link #commit} and
	 * {@link #rollback} more readable when used as an argument to those methods.
	 * Use <code>commit(Table.ALL)</code> rather than <code>commit(true)</code>, for
	 * example.
	 */
	...
	/***
	 * ********************************************************************** Create
	 * an unmodifiable table that contains selected rows from the current table. The
	 * {@link Selector} argument specifies a strategy object that determines which
	 * rows will be included in the result. <code>Table</code>.
	 *
	 * If the <code>other</code> argument is present, this methods "joins" all rows
	 * from the current table and the <code>other</code> table and then selects rows
	 * from the "join." If the two tables contain identically named columns, then
	 * only the column from the current table is included in the result.
	 * <p>
	 * Joins are performed by creating the Cartesian product of the current and
	 * "other" tables, using the Selector to determine which rows of the product to
	 * include in the returned Table. For example, If one table contains:
	 * 
	 * <pre>
	 *  a b
	 *  c d
	 ...
	 * <p>
	 * The <code>Table</code> returned from {@link #select} cannot be modified by
	 * you. The methods <code>Table</code> methods that normally modify the table
	 * (insert, update, delete, store) throw an
	 * {@link UnsupportedOperationException} if call them.
	 *
	 * @param where            a selector that determines which rows to include in
	 *                         the result. Use {@link Selector#ALL} to include all
	 *                         rows.
	 * @param requestedColumns columns to include in the result. null for all
	 *                         columns.
	 * @param other            Other tables to join to this one. At most three other
	 *                         tables may be specified. This argument must be null
	 *                         if you're not doing a join.
	 * @throws IndexOutOfBoundsException One of the requested columns doesn't exist
	 *                                   in either table.
	 *
	...
	/**
	 * Build a representation of the Table using the specified Exporter. Create an
	 * object from an {@link Table.Importer} using the constructor with an
	 * {@link Table.Importer} argument. The table's "dirty" status is cleared (set
	 * false) on an export.
	 * 
	 * @see #isDirty
	 */
	...
	/*******************************************************************
	 * Used for exporting tables in various formats. Note that I can add methods to
	 * this interface if the representation requires it without impacting the
