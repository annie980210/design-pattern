...
/**
 * A limited version of the Connection class. All methods undocumented
 * base-class overrides throw a {@link SQLException} if called.
 * <p>
 * Note that you can't mix non-autocommit behavior with explicit SQL
 * begin/commit statements. For example, if you turn off autocommit mode (which
 * causes a SQL begin to be issued), and then execute a SQL begin manually, a
 * call to `commit` will commit the inner transaction, but not the outer one. In
 * effect, you can't do nested transactions using the JDBC {@link #commit} or
...
public class JDBCConnection extends ConnectionAdapter {
	...
	/**
	 * Close a database connection. A commit is issued automatically if auto-commit
	 * mode is disabled.
	 * 
	 * @see #setAutoCommit
	 */
	...
	/**
	 * Terminate the current transactions and start a new one. Does nothing if
	...
	/**
	 * Roll back the current transactions and start a new one. Does nothing if
	 * auto-commit mode is on.
	 * 
	 * @see #setAutoCommit
	 */
	...
	/**
	 * Once set true, all SQL statements form a stand-alone transaction. A begin is
	 * issued automatically when auto-commit mode is disabled so that the
	 * {@link #commit} and {@link #rollback} methods will work correctly. Similarly,
	 * a commit is issued automatically when auto-commit mode is enabled.
	 * <p>
